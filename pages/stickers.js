...
const Stickers = ({ products }) => {
  return (
    <section className="text-gray-600 body-font">
      <Head>
        <title>Stickers - NextWear.com</title>
        <meta name="viewport" content="width=device-width, height=device-height, initial-scale=1.0, maximum-scale=1.0" />
      </Head>
      <div className="container px-5 py-24 mx-auto">
        <div className="flex flex-wrap -m-4 justify-center">
          {Object.keys(products).length === 0 && <p>Sorry all the Stickers are out of stock. New stock coming soon. Stay Tuned!</p>}
            ...
            <Link passHref={true} href={`/product/${products[item]?.slug}`} className="lg:w-1/4 md:w-1/2 p-4 w-full cursor-pointer shadow-lg mb-8" key={index}>
              <div>
                <div className="block relative rounded overflow-hidden">
                  <img alt="tshirt" className="object-cover w-full object-center md:h-[50vh] block" src={products[item]?.img} />
                </div>
                <div className="mt-4">
                  ...
                  <div className="mt-1">
                    ...
                    {products[item].color.includes('red') && <button className="border-2 border-gray-300 ml-1 bg-red-800 rounded-full w-6 h-6 focus:outline-none"></button>}
